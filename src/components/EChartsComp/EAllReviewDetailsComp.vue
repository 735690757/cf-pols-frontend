...
import {getAllCourseScoreRequest, getCourseScoreRequest} from "../../api/DataVisualAPIs.ts";
...
onMounted(async () => {
  ...
  const option = {
    title: {
      text: '学生评价详情',
      subtext: 'Details of student reviews',
      left: 'right'
    },
    tooltip: {
      trigger: 'item'
  ...
  await getAllCourseScoreRequest().then((res: any) => {
    const data = ref({
      score1: 0,
      score2: 0,
      score3: 0,
