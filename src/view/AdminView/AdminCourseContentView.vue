...
.content-core-top {
  background-color: rgba(202, 218, 255, 0.48);
  width: 270px;
  border-radius: 23px;
  padding-bottom: 20px;
  padding-top: 20px;
...
<template>
  <div class="edit-course-core">
    ...
    <div class="content-core">
      <div class="content-core-top"
           style="margin-bottom: 20px;display: flex;justify-content: center;align-items: center;">
        <div style="display: flex;justify-content: center;align-items: center;">
          推荐控制：
        </div>
        <el-switch @click="checkedRecEvent" inline-prompt size="large" v-model="checkedRec" active-text="已推荐"
                   inactive-text="未开启"/>
      </div>
      ...
      <div class="content-core-body">
        <div class="content-core-body-tool">
          <a-button disabled type="primary" style="background-color: rgba(103,61,61,0.16);cursor: not-allowed;"
                    class="tool-butt">
            <PlusOutlined/>
            添加小节
          </a-button>
          <a-button disabled type="primary" style="background-color: rgba(103,61,61,0.16);cursor: not-allowed;"
                    @click="opC1" class="tool-butt">
            <EditOutlined/>
            快速生成
          </a-button>
...
import {
  addRecommendCourseRequest,
  checkRecommendCourseRequest,
  removeRecommendCourseRequest
...
if (localStorage.getItem('course')) {
  try {
    ...
    if (courseData) {
      ...
      checkRecommendCourseRequest(course.value.id).then(res => {
        if (res.data.code === 200) {
          if (res.data.data) {
            checkedRec.value = true
          }
        }
      })
    }
...
const checkedRec = ref<boolean>(false);
const checkedRecEvent = () => {
  if (checkedRec.value) {
    addRecommendCourseRequest(course.value.id)
    elNotification("好的", "推荐课程成功", 'top-right')
  } else {
    removeRecommendCourseRequest(course.value.id)
    elNotification("好的", "取消推荐课程成功", 'top-right')
  }
}
</script>
